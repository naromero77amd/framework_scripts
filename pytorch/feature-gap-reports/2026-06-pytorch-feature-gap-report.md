...
> Nick's notes: We have a lot of implementations of SDPA, so it has been implemented many times at AMD, including AOTriton, CK, and even AITER. They are all implemented as (math) backends for PyTorch. Goal: complete the hipDNN integration into Pytorch so that the user can have access to feature-parity compared to cuDNN. TODO: David Peabody SDPA gap analysis AMD vs. Nvidia.
